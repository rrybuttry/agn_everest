...
if __name__=="__main__":
    ...
    plt.plot(time, flux_smooth, linewidth=10, color="#e6550d")
    plt.ylim(-.4,.4)
    plt.xticks([])
    plt.yticks([])
    plt.xlim(np.min(time), np.max(time))
    plt.tight_layout()
    plt.savefig("/Users/rachelbuttry/K2/submit/c8_channels/Channel%s.png"%channel, transparent=True, bbox_inches='tight', pad_inches=0)
